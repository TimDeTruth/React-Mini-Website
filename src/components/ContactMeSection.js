...
const LandingSection = () => {
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values, { resetForm }) => {
      submit("api/submit", values); // simulate API call
  ...
  // Listen to response changes to show alert
  useEffect(() => {
    if (response) {
      onOpen(response.type, response.message); // show the alert
      if (response.type === "success") {
        formik.resetForm(); // reset the form only on success
      }
    }
